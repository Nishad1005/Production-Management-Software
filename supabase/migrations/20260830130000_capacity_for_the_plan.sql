-- Kram — building the capacity grid for the plan, not for the whole masters.
--
-- ---------------------------------------------------------------------------
-- Measured on the live project, 30 Aug, and it is the engine, not the timeout.
--
--   run_schedule took 120.1s — canceling statement due to statement timeout
--
-- Exactly 120 seconds, so the per-function ceiling from 20260823100000 is in
-- force and the engine simply wants more than it. The runs that did complete,
-- back when only two of the fourteen departments were staffed, took **45
-- seconds to schedule 24 tasks**. Staffing the other twelve multiplied the
-- capacity grid and pushed it past the ceiling.
--
-- ---------------------------------------------------------------------------
-- Where the time goes.
--
-- `_cap_shift` is department × component × shift × working day, and each cell
-- is a call to `resolve_capacity`, which is five lookups — two on
-- capacity_overrides, one on component_rates, a correlated attendance
-- subquery, and machine_availability, itself a function. Every one of them
-- re-applies its row-level policy. Roughly 2.6 ms a cell.
--
-- And it built that grid from `_pair`, which is **every** department/component
-- pairing in the masters. At U&M that is 994, against the 168 that the twelve
-- orders in the book actually touch. Six times the grid for nothing: capacity
-- for a component nobody is making is written into the run and read by no
-- query in the system.
--
-- `_task` already holds exactly the pairings the scheduling walk looks up, and
-- it is built before this point. Sourcing the grid from it is the whole change.
--
-- Department-level output is unaffected: `schedule_department_day` sums
-- utilisation across components, and a component with no load contributed
-- zero. What disappears from `schedule_component_load` is rows reading
-- "capacity here, nothing planned on it", for components no order requires.
--
-- ---------------------------------------------------------------------------
-- What this does not fix, and must be fixed before U&M's real order book.
--
-- Per-cell `resolve_capacity` is still the shape of the cost, and it scales
-- with orders. Twelve provisional orders leave it near fifty seconds; a real
-- book is hundreds. The grid wants to be built set-based — hash joins against
-- capacity_overrides, component_rates and department_attendance instead of
-- five correlated lookups per cell. That is a rewrite of the engine's hottest
-- section and it is not going in the day before a demonstration. Recorded in
-- §6 as the next engine task, with the measurement that says how much it is
-- worth.
--
-- Regenerated from the text of 20260812130000, not retyped. Hand-rebuilding a
-- function from an earlier migration has twice reverted something silently —
-- `import_masters` on 18 Aug and `route_order_conflicts` on 30 Aug — and this
-- one is five hundred lines.
-- ---------------------------------------------------------------------------
