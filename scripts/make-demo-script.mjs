...
// What the live order entry in Part 2 will do to the headline figures — one
// new shipment line grows the plan by one job per department. Computed here so
// the script can promise "168 becomes 182" and be right on the day.
const linesAfter = orders.length + 1
const tasksAfter = run.task_count + activeDepartments.length
...
${screen({
  ...
  asks: [
    {
      q: 'Can we push an order to a different date?',
      a: `Two honest ways. To move one department's work a few days, drag its bar and pin it — the plan reshuffles around your decision. To move the whole container, change the shipment line's date on the Order book — remove the line, add it back with the new date — and re-plan. Deliberately two different acts, because they are two different promises: one is internal sequencing, the other changes what the customer was told.`,
    },
  ],
})}
...
${part(2, 'The life of one order', `The scenario the whole demonstration hangs on: a buyer calls with a new order. Check it, take it, and set the factory re-planning — then tour the floor while the engine thinks, and come back to watch the ripple.`)}
...
${screen({
  ...
  say: `“The order book — every promise the plan is built on. These twelve are practice orders we made up so you could see the software working; the moment your real orders go in, ours come out with one command.<br /><br />An order here is not one line: an order shipping in two containers is two separate problems with two separate dates, and the software treats it that way.<br /><br />Now — the scenario. Imagine your buyer has just called: <em>250 more of this chair, shipping mid-December.</em> Watch what taking that order properly looks like.”`,
})}

${screen({
  nav: 'Accept an order',
  title: 'First, ask whether we should say yes',
  mins: 3,
  caution: `<strong>The answer takes about ${runSecs + 5} seconds</strong> — it is genuinely re-planning the entire factory with the trial order added. Start it and keep talking; the paragraph below is written to fill exactly that wait. Never stand in silence.`,
  onScreen: `A product, a quantity, a ship date, and a Check button.`,
  doThis: `Pick the buyer's article, type 250, pick a date about three months out, press <em>Check</em> — and keep talking.`,
  say: `“Before anything is promised, we ask. Most systems answer this against an empty factory; this one pretends the order is real, replans everything with it in, and reports which departments would break — <em>given everything already promised to everyone else</em>. Then it throws the trial away. Nothing is committed by checking.<br /><br />While it thinks, notice what it is doing: recalculating roughly ${n(tasksAfter)} jobs across ${n(heatDepartments)} departments and every working day to January. It is the same full re-plan the planner runs each morning — which is exactly why the answer can be trusted.<br /><br />— and here is the verdict: department by department, <em>fits</em> or the reason it does not. If a department breaks, you know before the buyer hears yes, not after.”`,
})}

${screen({
  nav: 'Order book',
  title: 'The buyer confirms — enter it',
  mins: 3,
  onScreen: `The <em>Add an order</em> form: ERP order number, customer, article, quantity, confidence. It creates the first shipment line, where the container stuffing date goes.`,
  doThis: `Press <em>Add an order</em>. Order number <strong>PROV-DEMO-1</strong> — the PROV- prefix matters: it marks the order as practice data, so the standard clean-up removes it with everything else. Same article, quantity 250, confidence <em>Confirmed</em>, and the stuffing date you just checked. Save.`,
  say: `“Taking the order is a minute of typing: the order number from Panipuri, so the two systems always agree which order is which; the product; the quantity; and the date the container must be stuffed. That last date is the anchor — every deadline in the factory for this order will be calculated backwards from it.<br /><br />Once we have a sample of Panipuri's export file, even this minute disappears — orders will come in as a file instead of being typed.”`,
})}

${screen({
  nav: 'Command centre',
  title: 'Set the factory re-planning — and use the wait',
  mins: 1,
  onScreen: `The order book now shows <strong>${n(linesAfter)}</strong> lines — but every planning screen still shows the old plan, and says so. The plan does not change until it is re-run, deliberately.`,
  doThis: `Press <strong>Run the schedule</strong> (Confirmed + probable). It will read <em>Scheduling…</em> for about ${runSecs} seconds. Do not wait for it — walk straight into Part 3, and it will be finished long before you return.`,
  say: `“The new order is in the book, but notice — the plan has not moved. A plan that silently rearranged itself every time someone typed would be a plan nobody could trust for five minutes. Re-planning is a deliberate act, one button, and every version is kept.<br /><br />I have set it thinking — it recalculates every job in the factory, about ${runSecs} seconds. Rather than watch it, let me show you what the floor sees every day. When we come back, your order will be in the plan, and we will go and find it.”`,
})}

${part(3, 'The floor — while the engine thinks', `The screens for supervisors and department heads. You started the re-plan a moment ago; it completes in the background while you show these, which is why this part comes exactly here.`)}
...
${screen({
  ...
  doThis: `Declare a small quantity against any job — say 20 good, 1 rejected, with a cause. Then flick to <em>WIP</em>: the 20 you typed is already in that order's bar. Flick to <em>Quality</em>: the rejected piece has landed under its cause. One entry, three screens moved.`,
  say: `“This is the most important screen in the building, and the least glamorous. At the end of the day — or the shift — a supervisor writes down what was actually made: good pieces and rejected pieces, per product.<br /><br />Watch how far one entry travels. The moment I save it, the work-in-progress screen moves, the quality screen moves, and tomorrow's dashboard will compare it against what the plan asked for. Nothing is typed twice, anywhere, ever. It is built for a phone, thumb-sized, so it can be done standing on the floor.”`,
  asks: [
    {
...
${screen({
  ...
  mins: 2,
  skip: true,
  onScreen: `Every order, opened out across all ${n(heatDepartments)} departments: done, busy, waiting, not started — including the pieces you declared a minute ago.`,
...
${part(4, 'The ripple — go and find your order', `The re-plan you started in Part 2 finished while you toured the floor. Now walk back through three screens and let them find the order themselves. This is the part of the demonstration people remember.`)}

${screen({
  nav: 'Command centre',
  title: 'The two numbers moved',
  mins: 3,
  onScreen: `<strong>${n(linesAfter)}</strong> shipment lines where there were ${n(orders.length)}, and about <strong>${n(tasksAfter)}</strong> jobs where there were ${n(run.task_count)} — fourteen new jobs, one for each department your order passes through. The run panel shows today's time stamp.`,
  doThis: `Then take sixty seconds of silence and let <em>them</em> find PROV-DEMO-1: its bars on the <em>Schedule</em> (fourteen of them, ending at the date you typed), its load on the <em>Load heatmap</em> around those dates, and the order itself on <em>WIP</em>, marked still to begin. If the new order overloaded a day or cannot fit somewhere, it is on <em>Attention</em> — in front of everyone, which is the point.`,
  say: `“Remember the two numbers from the start — ${n(orders.length)} shipments, ${n(run.task_count)} jobs. Now look: ${n(linesAfter)}, and ${n(tasksAfter)}. The order you watched me type five minutes ago has been broken into a job for every department, each with its own deadline counted back from the container date, each already weighed against everything else you have promised.<br /><br />That is the whole system in one sentence: <strong>type the promise once, and every screen in the building rearranges itself around it</strong> — the calendar, the daily loads, the department work lists, the buying dates, and the warnings if it does not fit.<br /><br />And if it had <em>not</em> fit, we would not have found out from an angry buyer in December. It would be a red sentence on the Attention screen today, while every option is still open.”`,
  asks: [
    {
      q: 'How do we take it out again?',
      a: `The × against the order on the Order book removes it, and the next re-plan forgets it. This one is named PROV-, so the standard clean-up that removes all our practice data takes it too. Nothing you saw today needs to be un-done by hand.`,
    },
  ],
})}
...
${screen({
  nav: 'What if',
  title: 'And if it had not fit — try the fix on a copy',
  mins: 2,
  skip: true,
  onScreen: `A department, a date range, and a dial for more or less capacity. The result appears beside the live plan.`,
  say: `“Suppose your new order <em>had</em> overloaded stitching in November. This is where the argument happens: put four more people in stitching for a fortnight — on a copy — and see what it does to every date in the factory. Or take a week out for maintenance and see what it costs before committing to it.<br /><br />The real plan is untouched until you decide to keep the result, and every scenario is kept with a label, so next month you can see what was considered and why.”`,
})}

${part(5, 'Money and material', `Two screens that are built, tested, and empty — because the data behind them is item 4 on the sheet you are leaving with them. Say that with confidence, not apology.`)}
...
${screen({
  ...
  say: `“This screen takes the plan and turns it into purchasing: what the factory is committed to buying, week by week, and — in red — anything already past the date it should have been ordered, counting backwards from when production needs it. The order you entered ten minutes ago would appear here too, as the foam and fabric and fittings it will consume.<br /><br />It is empty for one reason only: it needs to know what each product is made of, and nobody has given us that yet. Give us the parts list for <em>one</em> product and this screen switches on for it the same day.”`,
})}
...
${part(6, 'Where the numbers live, and the close', `End on the two honest screens: the sheet every other number came from, and the screen that refuses to guess.`)}
...
<div class="doc">
  ...
  <header class="head">
    ...
    <dl class="stamp">
      ...
      <dt>Full run</dt><dd>~45 min · short version ~25 (skip the tagged beats; keep the order scenario)</dd>
    </dl>
  </header>
